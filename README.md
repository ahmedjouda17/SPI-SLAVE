...
* **Step 2 (Read Data):**
  ...
  * The RAM retrieves the stored byte from the latched address, passes it to the SPI Slave (`tx_data`), and the PISO (Parallel-In Serial-Out) shift register shifts the data bit-by-bit back to the master through the `MISO` line.
...
* 💾 **Single-Port RAM:** A 256×8-bit internal data storage element. It allows a single read or write access at a time, controlled entirely by the SPI Slave.
